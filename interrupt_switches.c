...
__interrupt void switchP4_interrupt(void){
  ...
  if (P4IFG & SW1) {
    ...
    */
    ...
    //temporarily disable switch interrupts by masking out the interrupt enable bits
    ...
    //    SW1_debug_counter++;
    //
    //    HEXtoBCD(SW1_debug_counter);
    //    adc_line(3,0);
    //    display_changed = 1;
    */

    TB0CCTL1 &= ~CCIFG; // 3) clear any possible pending interrupt flags/faults
    TB0CCR1 += TB0_SEC_0_050; // 4) Note: needs to be for 1 second for safe debounce
    // 4) Alternate way: TB0CCR1 = TB0R + CCR1_200_MSEC_INTERVAL;
...
__interrupt void switchP2_interrupt(void){
  ...
  if (P2IFG & SW2) {
    ...
    TB0CCTL2 &= ~CCIFG; // 3) clear any possible pending interrupt flags
    TB0CCR2 += TB0_SEC_0_050; // 4) Note: needs to be for 1 second for safe debounce
    // 4) Alternate way: TB0CCR1 = TB0R + CCR0_200_MSEC_INTERVAL;
